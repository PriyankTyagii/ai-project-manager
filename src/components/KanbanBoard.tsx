...
function Column({ title, icon, tasks, color, onTaskClick }: ColumnProps) {
  const styles = {
    ...
    red: { bg: "bg-red-50", border: "border-red-200", text: "text-red-700", headerBg: "bg-red-100" },
    ...
    <div className={`${style.bg} rounded-2xl border-2 ${style.border} overflow-hidden`}>
      <div className={`${style.headerBg} px-4 py-4 border-b-2 ${style.border}`}>
        <div className={`flex items-center justify-between ${style.text}`}>
          ...
          <span className="bg-white px-3 py-1 rounded-full text-sm font-bold">{tasks.length}</span>
        </div>
      </div>
      <div className="p-4 space-y-3 min-h-[400px]">
        {tasks.length === 0 ? (
          <div className="text-center py-12">
            <div className={`${style.text} opacity-50 text-sm`}>No tasks yet</div>
          </div>
        ) : (
          tasks.map((task) => (
      ...
      <div className="bg-white rounded-3xl max-w-3xl w-full max-h-[90vh] overflow-y-auto shadow-2xl" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="bg-gradient-to-r from-indigo-500 to-purple-600 px-8 py-6 flex justify-between items-start">
          <div className="flex-1 pr-4">
            ...
            {task.epic && (
              <span className="inline-block bg-white/20 backdrop-blur-sm text-white px-3 py-1 rounded-full text-sm font-medium">{task.epic}</span>
            )}
          </div>
          <button onClick={onClose} className="bg-white/20 hover:bg-white/30 backdrop-blur-sm p-2 rounded-full transition-colors">
              ...
              {task.agentComments.map((comment, idx) => (
                <div key={idx} className="bg-gradient-to-r from-indigo-50 to-purple-50 p-4 rounded-xl border border-indigo-200">
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <span className="text-xs text-indigo-600 bg-white px-2 py-1 rounded-full">{comment.type}</span>
                  </div>
        ...
        <div className="flex gap-3">
          <ActionButton onClick={() => handleStatusChange("backlog")} label="Backlog" color="gray" />
          <ActionButton onClick={() => handleStatusChange("in_progress")} label="In Progress" color="blue" />
          <ActionButton onClick={() => handleStatusChange("done")} label="Done" color="green" />
        </div>
      </div>
    </div>
  );
...
function ActionButton({ onClick, label, color }: ActionButtonProps) {
  const colors = {
  ...
  return (
    <button onClick={onClick} className={`flex-1 px-6 py-3 rounded-xl font-semibold transition-all ${colors[color]}`}>
      {label}
    </button>
  );
}
