...
export async function DailySummary({ projectId }: { projectId: string }) {
  ...
  const report = latestReport.payload as ReportPayload;
    ...
    <div className="bg-white rounded-2xl border-2 border-gray-200 overflow-hidden shadow-lg">
      ...
      <div className="bg-gradient-to-r from-indigo-500 to-purple-600 px-6 py-4">
        <div className="flex items-center space-x-3">
          ...
          <h3 className="font-bold text-white text-lg">Today&apos;s Summary</h3>
        </div>
      </div>
          ...
          <div className="bg-gradient-to-r from-indigo-50 to-purple-50 rounded-xl p-4 border border-indigo-200">
            ...
            <div className="space-y-2">
              {report.insights.slice(0, 2).map((insight: string, idx: number) => (
                <p key={idx} className="text-xs text-gray-700 leading-relaxed pl-6 relative">
...
function MetricRow({ icon, label, value, color }: any) {
  ...
  return (
    <div className={`flex items-center justify-between p-3 rounded-xl border-2 ${colors[color]}`}>
      <div className="flex items-center space-x-3">
